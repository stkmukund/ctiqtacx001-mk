...
export const useFormStore = defineStore("form", () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!validationResult.success && formValues.value.paymentMethod !== "PAYPAL") {
      ...
      const apiResponse = await callPaayAuthorize() as { token: string | null };
      if (apiResponse.token) {
        const browserresponse: any = await runPaayBrowserAuthentication(apiResponse.token);
        if (browserresponse.status === 'SUCCESS' && browserresponse.data) {
          const { transactionId } = browserresponse.data;
          const transactionresponse: any = await run3dsTransectionUpdates(apiResponse.token, transactionId);
          if (transactionresponse.status === 'Y' || transactionresponse.status === 'N') {
            formValues.value.cavv = transactionresponse.authenticationValue || "";
            formValues.value.eci = transactionresponse.eci || "";
            formValues.value.xid = transactionresponse.dsTransId || "";
            formValues.value.version = transactionresponse.protocolVersion || "2.2.0";
            formValues.value.acsTransId = transactionresponse.acsTransId || '';
            formValues.value.status = transactionresponse.status || "N";

            // Store these values in sessionStorage
            sessionStorage.setItem("cavv", formValues.value.cavv);
            sessionStorage.setItem("eci", formValues.value.eci);
            sessionStorage.setItem("xid", formValues.value.xid);
            sessionStorage.setItem("version", formValues.value.version);
            sessionStorage.setItem("cardNumber", formValues.value.cardNumber);
            sessionStorage.setItem("cardMonth", formValues.value.expiryMonth);
            sessionStorage.setItem("cardYear", formValues.value.expiryYear.length > 2 ? formValues.value.expiryYear.slice(-2) : formValues.value.expiryYear);
            sessionStorage.setItem("acsTransId", formValues.value.acsTransId);
            sessionStorage.setItem("status", formValues.value.status);
          }
        }
      }
