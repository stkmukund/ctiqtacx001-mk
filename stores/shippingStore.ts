...
export const useShippingStore = defineStore("shipping", () => {
  ...
  const updateShipping = async () => {
    if (allShippingStates.value.length > 0) {
      ...
      else if (formStore.formValues.shippingMethod === config.public.freeShipProfileId.toString()) formStore.formValues.shippingMethod = config.public.freeShipProfileId.toString();
    }
    cartStore.updateLoading(false, 'shipping');
  }
